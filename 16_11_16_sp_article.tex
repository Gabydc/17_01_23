...
Iterative methods are known to be the best option to solve large-scale problems. However, sometimes the iterative methods are not sufficient to solve the problems in a reasonable amount of time and preconditioners become necessary to accelerate the convergence of these methods \cite{Benzi02}. As the systems become larger or ill-conditioned, new preconditioning techniques need to be developed to improve the performance of an iterative method \cite{Vuik02}.\\
...
$porosity$ ($\phi$).\\
Some rocks are compressible and their porosity depends on the pressure, the dependence is called 
...
Rock porosity is related to the pressure through the rock compressibility, the relation is given by the following expression:
\begin{equation*}
 c_r=\frac{1}{\phi}\frac{d\phi}{dp}=\frac{dln(\phi)}{dp},
\end{equation*}
...
\begin{equation*}\label{eq:fc}
 c_f=\frac{1}{\rho}\frac{d\rho}{dp}=\frac{dln(\rho)}{dp}.
\end{equation*}
...
If the density and the porosity do not depend on the pressure in Equation \eqref{eq:ce1}, we have an incompressible model. Assuming no gravity terms and a fluid with constant viscosity, Equation \eqref{eq:ce1} becomes:
\begin{equation}\label{eq:cel}
-\frac{\mathbf{\rho}}{\mu}\nabla \cdot \left({K} \nabla p\right)=q.
\end{equation}
\emph{Discretization}\\
The spatial differentials are approximate using a finite difference scheme with cell central 
derivatives. For a 3D model, taking a mesh with a uniform grid size $\Delta x$, $\Delta y$, $\Delta z$ where $(i,j,l)$ is the center 
of the cell
...
&=\frac{ k_{i+\frac{1}{2},j}(p_{i+1,j,l}-p_{i,j,l})-k_{i-\frac{1}{2},j,l}(p_{i,j,l}-p_{i-1,j,l})}{\left( \Delta x\right)^2}+\mathscr{O}(\Delta x^2),
\end{align*}
where $k_{i-\frac{1}{2},j,l}$ is the harmonic average of the permeability for the cells 
$(i-1,j,l)$ and $(i,j,l)$:
\begin{equation}\label{eq:ha}
 k_{i-\frac{1}{2},j,l}=\frac{1}{\frac{1}{ k_{i-1,j,l}}+\frac{1}{ k_{i,j,l}}}.
\end{equation}

After discretization, Equation \ref{eq:cel} can be written as:
 \begin{equation}\label{eq:cel1}
\mathbf{T}\mathbf{p} = \mathbf{q},
\end{equation}
Where $\mathbf{T}$ is known as the transmissibility matrix with elements in adjacent grid cells, e.g.,
the $transmissibility$ ($T_{i-\frac{1}{2},j,l}$) between grid cells $(i-1,j,l)$ and $(i,j,l)$  is defined as:
\begin{equation}\label{eq:htrans}
 T_{i-\frac{1}{2},j,l}=\frac{\Delta y}{\Delta x}\frac{d}{\mu}k_{i-\frac{1}{2},j,l},
\end{equation} 
System \eqref{eq:cel1} is a linear system that can be solved with iterative or direct methods.\\\\
\textbf{Compressible fluid}\\
If the fluid is compressible with a constant compressibility, the density depends on the pressure (see Equation \ref{eq:rhoeq}). Therefore, Equations \ref{eq:ce} become:
...
Equation \ref{eq:ce4} can be discretized in space, using, e.g., finite differences schemes. After discretization Equation \ref{eq:ce4} reads:
...
{q}_{(i,j,l)}={I}_{(i,j,l)}({p}_{R(i,j,l)}-{p}_{bhp(i,j,l)}),
\end{equation}
where ${I}_{(i,j,l)}$ is the productivity or injectivity index of the well, ${p}_{R(i,j,l)}$ is the reservoir pressure in the cell 
where the well is located, 
and ${p}_{bhp(i,j,l)}$ is the pressure inside the well. \\\\
\emph{Incompressible fluid}\\
Using the well model for an incompressible fluid, Equation \ref{eq:cel1} transforms into:
...
For a compressible fluid, using the well model, Equation \ref{eq:ce5} reads:
...
To solve Equations \eqref{eq:celw1} and \eqref{eq:cew4}, it is necessary to define boundary conditions.  These conditions can be prescribed pressures 
(Dirichlet conditions), flow rates (Neumann conditions) or a combination of these (Robin conditions). \\For Equation 
\eqref{eq:cew4} we also need to specify the initial conditions that are the pressure values of the reservoir at the beginning of the simulation.\\
...
$$\mathbf{J}(\mathbf{p}^k)\delta\mathbf{p}^{k+1}=-\mathbf{F}(\mathbf{p}^k),
\qquad \mathbf{p}^{k+1}=\mathbf{p}^k+\delta \mathbf{p}^{k+1},$$
where $\mathbf{J}(\mathbf{p}^k)=\frac{\partial \mathbf{F}(\mathbf{p}^k)}{\partial \mathbf{p}^k}$ is the 
...
The procedure to solve this problem consists of three stages. During the first stage, we select a 
time and solve Equation \ref{eq:cew4} for this particular time, i.e., we have a solution for each time step. In the second stage, 
we linearize the equations with the NR method, i.e., we perform a series of iterations to find the 
zeros of Equation \eqref{eq:NR}. For every NR iteration the linear system in Equation \eqref{eq:lsJ} is solved. A summary of 
...
Since $\mathbf{A}$ is SPD we choose as iterative method the Conjugate Gradient (CG) method accelerated by the Incomplete Cholesky preconditioner. In this work, we will also study the acceleration with deflation techniques. In this section we give a brief overview of the methods. \\\\
...
$\mathcal{K}_k(\mathbf{A},\mathbf{r}^0)=span\{\mathbf{r}^0,\mathbf{A}\mathbf{r}^0,\dots,\mathbf{A}^{k-1}\mathbf{r}^0\}$ and $\mathbf{x}^k\in \mathbf{x}^0+\mathcal{K}_k(\mathbf{A},\mathbf{r}^0)$ is minimal for all approximations contained in $\mathbf{x}^0+\mathcal{K}_k(\mathbf{A},\mathbf{r}^0).$ The error of this approximation is bounded by:
...
\end{equation}
The new system has the same solution but can provide a substantial improvement on the spectrum. 
...
\end{equation}
$\mathbf{M}$ is chosen as an $SPD$ matrix such that $\mathbf{\kappa}(\mathbf{M}^{-1}\mathbf{A})\leq \mathbf{\kappa}(\mathbf{A}),$ and $\mathbf{M}^{-1}b$ is cheap to compute.\\\\
\textbf{Deflation}\\
...
\end{equation}
Multiplying expression \eqref{eq:splx} by $\mathbf{A}$, using the properties above, we have:
\begin{align*}
...
components of the null space of $\mathbf{P}\mathbf{A}$. A unique solution of this system, called the deflated
...
For this, we need to choose the way of selecting these snapshots. The idea behind this selection is to obtain a small number of snapshots and, at the same time,
...
\textbf{Lemma 1.} 
Let $\mathbf{A} \in \mathbb{R}^{n\times n}$ be a non-singular matrix, and $\mathbf{x}$ is a solution of:
...
If we use an iterative method to obtain an approximate solution $\mathbf{x}^k$ for the system $\mathbf{A}\mathbf{x}=\mathbf{b},$ we cannot compute the relative error $e_r$ (Equation \ref{eq:re}) of the approximation with respect to the true solution because the true solution is unknown, 
...
where $\mathbf{x}_i^{1(i)}$ is the $i-th$ approximated solution of the snapshot $i$ after $1$ iteration.\\ 
...
and if the accuracy of the snapshots used as deflation vectors is high enough (see Section \ref{accs}), the solution is expected to be achieved in one iteration. 
...
We propose the use of snapshots and the snapshots-based POD basis vectors as deflation vectors for the above-mentioned method.\\
...
We also propose the use of a linearly independent basis of POD as deflation vectors, obtained from the previously computed snapshots for the incompressible and compressible cases.
...
We simulate single-phase flow through a porous medium, for an incompressible fluid (see Equation \ref{eq:cel1}), with the previously mentioned characteristics. Homogeneous Neumann boundary conditions are imposed on all boundaries. \\
A set of four linearly independent snapshots is used as deflation vectors. We also use a linearly dependent set of 15 snapshots and a basis of POD (linearly independent set) obtained from these 15 snapshots. We set the same boundary conditions as in the original problem for all the snapshots.
The four linearly independent snapshots ($\mathbf{z}_1$-$\mathbf{z}_4$) are obtained giving a value of zero to one well and non-zero values to the other wells, such that the sum of the well pressures equals zero. The set of 15 snapshots are all the possible combinations of wells that satisfy that the flow in equals the flow out of the reservoir. The snapshots and the solutions are obtained with a tolerance of $10^{-11}$. \\
A summary of the configurations is presented in Table \ref{table:sc}.
...
\end{minipage}\caption{Table with the well configuration of the system and the snapshots.}
...
For the deflation vectors of DICCG$_{POD}$ we plot the eigenvalues of the snapshot correlation matrix $\mathbf{R}=\mathbf{X}^T \mathbf{X}$ (see section \ref{POD}) in Figure \ref{fig:eig}. We observe that there are 4 eigenvalues much larger than the rest, these largest eigenvalues are responsible for the divergence of the method. For the  DICCG$_{POD}$ method we use the eigenvectors corresponding to the larger eigenvalues as deflation vectors.\\
\renewcommand{\arraystretch}{1.3}
\begin{table}[!ht]\centering
\begin{minipage}{.7\textwidth}
\vspace{10pt}
...
\end{wrapfigure}
This model has large variations in the permeability coefficients, the contrast between coefficients is $3\times 10^7$.
The model contains 60 x 220 x 85 cells (Figure \ref{fig:permcc}) and five wells positioned as in the previous experiment.
...
For the deflated method with 15 linearly dependent snapshots (DICCG$_{15}$), we observe that the desired accuracy is not reached after 2000 iterations, as the deflation methods are linearly dependent, the deflation method is unstable because the matrix $E$ is a nearly singular matrix (new report).
...
\centering 
\vspace{-20pt}
\includegraphics[width=5cm,height=5cm,keepaspectratio]{/home/wagm/cortes/Localdisk/Results/sp_article/size_70perm_1_5wells_c_1e-3_s_52upddv_10pod/perm.jpg}
 \vspace{-15pt}
...
according to Equation \ref{eq:rhoeq}. We solve Equation \eqref{eq:ce6} for a fluid with the a compressibility of $c= 1 \times 10^{-3}$.
...
well is placed in the center. The length of the domain is 70 m and the grid size is 35 grid cells in each dimension.  We use homogeneous Neumann boundary conditions on all boundaries. \\
The initial pressure of the reservoir is set as 200 bars. The bottom hole pressure (bhp) in the corner wells is 100 bars and 
...
\vspace{7mm}
\caption{Eigenvalues of the data snapshot correlation matrix $\mathbf{R}=\mathbf{X}\mathbf{X}^T$, contrast between permeability layers of $10^{-1}$.}
\label{fig:eig_POD_1}
\end{minipage}
\end{figure}
As mentioned before, for each time step, the previous 10 solutions are used as snapshots to compute the POD basis. The eigenvalues of the snapshot correlation matrix $\mathbf{R}=\frac{1}{m}\mathbf{X}\mathbf{X}^T$ constructed with the previous ten time steps are presented in Figure \ref{fig:eig_POD_1} for the 20$^{th}$ time step. In Figure \ref{fig:eig_POD_1}, we observe that six eigenvalues are larger than the rest. Then, we use the eigenvectors corresponding to these six eigenvalues as deflation vectors (DICCG$_{POD_6}$). For a contrast between permeability layers of $10^{-2}$ and $10^{-3}$ we have 7 larger eigenvalues, therefore, we use 7 POD basis vectors as deflation vectors (DICCG$_{POD_7}$).  
...
$10^{-1}$ &DICCG$_{10}$&780 & 140&42& 182&23\\
 &DICCG$_{POD_6}$& &&84&  224&29\\
 \hline
$10^{-2}$  &DICCG$_{10}$& 624&100&42&  142&23\\
          &DICCG$_{POD_7}$&&&42& 142 &23\\
          \hline
 $10^{-3}$ &DICCG$_{10}$& 364&20&42  &62& 17\\
          &DICCG$_{POD_7}$&&&42&  62&17 \\
 \hline
 \end{tabular}
...
&DICCG$_{POD_6}$& &&198&  378& 38\\
\hline
$10^{-2}$&DICCG$_{10}$& 832&140&90&  230&28\\
&DICCG$_{POD_7}$&&&154&  294&33\\
\hline
$10^{-3}$&DICCG$_{10}$&884&110&90&  200&23 \\
&DICCG$_{POD_7}$& &&150&  260& 29\\
 \hline
 \end{tabular}
...
988 linear iterations to compute the solution for the 52 time steps. By contrast, when we use the deflated methods,
we need 180 linear iterations to compute the snapshots during the first ten time steps with the ICCG method and 78 and 198 for the 42
remaining time steps with the DICCG$_{10}$ and DICCG$_{POD_6}$ methods. Therefore we need in total 258 and 378 linear
...
for DICCG$_{10}$ and 294 for the DICCG$_{POD_7}$ method. That means the 28\% and 33\% of the
ICCG iterations.
For a contrast between permeability layers of $10^{-3}$, we require 884 linear iterations for the 52 time steps.
For the DICCG$_{10}$ and DICCG$_{POD_7}$ methods, we require 200 and 260 iterations. That is 23\%
...
We study the complete SPE 10 benchmark that consists of $60\times220\times85$ grid cells and has a contrast in permeability of $3\times 10^{7}$. To solve the linear system obtained after the NR linearization, we use 10 snapshots (the previous 10 time step solutions), and POD basis vectors as deflation vectors. \\
...
\hline
Method& Total & ICCG&DICCG &Total&\% of total\\
   & ICCG (only) & Snapshots& &ICCG+DICCG& ICCG\\
\hline

DICCG$_{10}$&&&1134&2904&28 \\
DICCG$_{POD_4}$ &10173&1770&1554&3324&32  \\
\hline
 \end{tabular}
...
Method& Total & ICCG&DICCG &Total&\% of total\\
   & ICCG (only) & Snapshots& &ICCG+DICCG& ICCG\\
\hline
DICCG$_{10}$&10231&1830&200&2030&20 \\
DICCG$_{POD_4}$ &&&200&2030&20 \\
...
We also observe that for the first NR iteration we need 1770 linear iterations to compute the ten initial snapshots (computed with ICCG) and 1134 to compute the solution of the rest of the solutions (computed with DICCG). For the second NR iteration, the number of linear iterations is 1830 for the ten initial snapshots and 200 for the deflated methods. This shows that the largest amount of work is carried out for the computation of the snapshots, which is more evident for the second NR iteration.
...
In this work, we combine preconditioning with deflation and POD methods to accelerate the 
convergence of iterative methods for large systems and systems with high-contrast in permeability. 
In particular, we study the Conjugate Gradient (CG) method preconditioned with Incomplete Cholesky. 
However, the deflation techniques here presented are not restricted to these methods and could be 
...
With DICCG we reduce the number of iterations necessary to achieve convergence to around 25\% of 
the number of iterations necessary for the ICCG method. We observe that the performance of the 
...
However, only a limited number of deflation vectors is necessary to obtain a good speed-up 
